...
export default function DemoPage() {
  ...
  const t = lang === "fr" ? {
    back: "Retour", title: "Demo", sub: PRODUCT + " — Q&A documentaire avec citations",
    desc: "Indiquez une collection (ex: contrats_clients, politique_RH) et posez une question. L'agent RAG genere une reponse avec sources citees. Aucune connexion reelle a un vector store — POC qui illustre la logique de production.",
    inputLabel: "Votre requete",
    placeholderCol: "Collection indexee (ex: contrats_clients_2025)",
    placeholderQ: "Votre question (ex: Quel est le delai de preavis ?)",
    generate: "Interroger le corpus", generating: "Recherche en cours...",
    briefTitle: "Reponse + sources", emptyHint: "La reponse s'affiche ici une fois generee.",
    mockPinecone: "Indexer dans Pinecone", mockWeaviate: "Indexer dans Weaviate",
    mockNotion: "Ingestion Notion", mockDrive: "Ingestion Google Drive",
    sentPinecone: "Collection envoyee a Pinecone (mode demo, pas d'index reel cree)",
    sentWeaviate: "Collection envoyee a Weaviate (mode demo, pas d'index reel cree)",
    sentNotion: "Workspace Notion connecte (mode demo, pas d'OAuth reel)",
    sentDrive: "Google Drive connecte (mode demo, pas d'OAuth reel)",
    fallback: "Mode statique : la cle LLM sera ajoutee au prochain deploiement.",
    poweredBy: "Modele :",
    note: "DEMO POC — pas d'index Pinecone/Weaviate, pas d'OAuth Notion/Drive. L'IA simule la couche RAG avec sources inventees pour la demonstration.",
  } : {
    back: "Back", title: "Demo", sub: PRODUCT + " — document Q&A with citations",
    desc: "Specify a collection (e.g. customer_contracts, hr_policy) and ask a question. The RAG agent returns an answer with cited sources. No real vector store connection — POC showing the production logic.",
    inputLabel: "Your query",
    placeholderCol: "Indexed collection (e.g. customer_contracts_2025)",
    placeholderQ: "Your question (e.g. What is the notice period?)",
    generate: "Query the corpus", generating: "Searching...",
    briefTitle: "Answer + sources", emptyHint: "The answer will appear here once generated.",
    mockPinecone: "Index in Pinecone", mockWeaviate: "Index in Weaviate",
    mockNotion: "Ingest from Notion", mockDrive: "Ingest from Google Drive",
    sentPinecone: "Collection sent to Pinecone (demo mode, no real index created)",
    sentWeaviate: "Collection sent to Weaviate (demo mode, no real index created)",
    sentNotion: "Notion workspace connected (demo mode, no real OAuth)",
    sentDrive: "Google Drive connected (demo mode, no real OAuth)",
    fallback: "Static mode: LLM key will be added at next deploy.",
    poweredBy: "Model:",
    note: "DEMO POC — no Pinecone/Weaviate index, no Notion/Drive OAuth. The AI simulates the RAG layer with invented sources for demonstration.",
  };

  async function generate() {
    setError(""); setBrief(""); setModel(""); setStaticMode(false);
    if (!question.trim()) {
      setError(lang === "fr" ? "Entrez une question." : "Enter a question.");
      return;
    }
    setLoading(true);
    try {
    ...
    <div style={{ minHeight: "100vh", background: PAL.bg, color: PAL.txt1, display: "flex", flexDirection: "column" }}>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .wk-textarea { width: 100%; padding: 12px 14px; border-radius: 10px; background: ${PAL.surface}; border: 1px solid ${PAL.border}; color: ${PAL.txt1}; font-family: inherit; font-size: 14px; resize: vertical; min-height: 90px; transition: border-color .2s, background .2s; }
        ...
        <div className="demo-grid" style={{ display: "grid", gridTemplateColumns: "360px 1fr", gap: 24 }}>
          <section style={{ background: PAL.surface, border: `1px solid ${PAL.border}`, borderRadius: 16, padding: 22 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 18 }}>
              <input className="wk-input" value={collection} onChange={(e) => setCollection(e.target.value)} placeholder={t.placeholderCol} />
              <textarea className="wk-textarea" value={question} onChange={(e) => setQuestion(e.target.value)} placeholder={t.placeholderQ} />
            </div>
          ...
          <section style={{ background: PAL.bg2, border: `1px solid ${PAL.border}`, borderRadius: 16, padding: 22, minHeight: 420, display: "flex", flexDirection: "column" }}>
            ...
            {brief && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 18, paddingTop: 18, borderTop: `1px solid ${PAL.border}` }}>
                <button className="wk-btn-ghost" onClick={() => showToast(t.sentPinecone)}>🌲 {t.mockPinecone}</button>
                <button className="wk-btn-ghost" onClick={() => showToast(t.sentWeaviate)}>🧬 {t.mockWeaviate}</button>
                <button className="wk-btn-ghost" onClick={() => showToast(t.sentNotion)}>📝 {t.mockNotion}</button>
                <button className="wk-btn-ghost" onClick={() => showToast(t.sentDrive)}>💾 {t.mockDrive}</button>
              </div>
            )}
